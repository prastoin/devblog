...
const BaseAnchor: React.FC<BaseAnchorProps> = ({ label }) => {
    const encodedId = label.toLowerCase().split(' ').join('-');

    return (
        <a
            className="group font-bold inline text-black dark:text-white hover:text-black dark:hover:text-white hover:no-underline"
            id={encodedId}
            href={`#${encodedId}`}
        >
            ...
            <svg
                className="inline ml-2 w-4 h-4 lg:w-6 lg:h-6 underline md:invisible group-hover:visible "
