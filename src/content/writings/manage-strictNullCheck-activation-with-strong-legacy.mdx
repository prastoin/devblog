---
title: Approaching massive volumes of TypeScript errors
...
- Testing, freezing the state of the application
- Deadcode, using for example [knip](https://knip.dev/), we don't wanna fix ts-errors on anything unused.
- Outgoing and incoming data, following API contracts.

Afterwards becomes more thinkable to step by step refactor features by features, define benefic routine within your team to avoid adding new legacy to the codebase and in parallel of development fix the old one as possible.
...
I also want to ts-server to shout at me when he detects anything I haven't been thinking of, often intentionally when I want him to spread an interface mutation to guide me on what now needs to be done.
There's so many framework and tools in the ecosystem, TypeScript is layer now widely shared between all of them, allowing to expose a meaningful user interface to anyone understand TypeScript agnostically of the stack under the hood.
...
Also exist great tools such as that allow centralizing and transpiling an contract API through a `TypeScript` utils. That could be shared between the front and the back.

- Zodios
- ts-rest
- Trpc

For handling legacy, or when you don't have the hand on the back or maybe it's just not a Node.js one. Zodios might be your deal.
Following this great tool `zodios-to-client` that will generate a Zodios instance that you could only import front side based on the OpenApi specification of the API you're dealing with.
This might really be a time gain/boost if you're handling massive volume of endpoints.
Note that it's ship with `Zod` that could allow runtime type validation :eyes:
