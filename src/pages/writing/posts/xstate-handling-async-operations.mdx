...
Lets think about a machine that when interpreted instantly send an HTTP request to a server in order to retrieve a user instance.
...
From there we can imagine a machine that starts within 2 parallel states that both waits for a specific events to transition to their final state.
Lets say a root parent state that has two states child, one being the parallel states handler and the other being the machine final state nor further logic.
Inside the parallel machine would be found two parallel states waiting for their specific ressources, here `1` and `2`, to be fulfilled by the user before reaching their final states.

A parallel parent state can listen its parallel childs final state transitions through the [onDone](https://xstate.js.org/docs/guides/final.html#parallel-states) property.
...
On every promises validation we transtition thanks to the parent parallel state node `onDone` property to a further state.
...
`User pressed load user data button` for starting loading everything and `User pressed load user data button` for reseting the machine context and state.
